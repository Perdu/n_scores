...
def top_player_graph(rows, max_scores_per_year):
    table_top_players = {}
    year_i = 0
    year_cur = 2006
    players = {}
    ...
    for row in rows:
        year = int(row[1])
        player = row[2]
        if year == year_cur and year_i >= max_scores_per_year and player not in players:
            continue
        if year != year_cur:
            year_cur = year
            year_i = 0
...
def disp_stats():
    ...
    top_players = top_player_graph(rows, max_scores_per_year=10)
    # top players graph (top 20)
    rows = execute("SELECT COUNT(*) as c, YEAR(timestamp) as y, pseudo FROM score_unique GROUP BY YEAR(timestamp), pseudo order by y, c DESC;", return_rows = True)
    top_players_20 = top_player_graph(rows, max_scores_per_year=10)
    return render_template("stats.html", table=table, scores_series=scores_series, top_players=top_players, top_players_20=top_players_20)
